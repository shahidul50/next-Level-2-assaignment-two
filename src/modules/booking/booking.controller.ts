    ...
    try {
      ...
      const responseData = {
        ...
        vehicle: {
          ...
          daily_rent_price: selectedVehicleDetails.rows[0].daily_rent_price,
        },
      };
...
const updateBooking = async (req: Request, res: Response) => {
  if (req.body !== undefined) {
    ...
    const bookingId = req.params.bookingId ? req.params.bookingId : "0";
    ...
    try {
      ...
      if (
        isCustomer &&
        getDaysBetween(rentStartDate, new Date().toString()) > 0
      ) {
        return SendError(
          res,
          403,
          false,
          "Forbidden",
          "You cannot change status after start date"
        );
      }
